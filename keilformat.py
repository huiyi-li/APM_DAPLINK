...
def parse_uvprojx(file_path, project_root):
    ...
    # 清理路径和定义的空格
    include_paths = [p.strip().replace('\\', '/') for p in include_paths if p.strip()]
...
def parse_absolute_uvprojx(file_path, project_root):
    # 解析XML文件
    tree = ET.parse(file_path)
    root = tree.getroot()
...
def generate_absolute_compile_commands(path):
    ...
    include_paths, defines, source_files = parse_absolute_uvprojx(uvprojx_path, project_root)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', '-p', required=False, help='Specify the path of .uvprojx file example "-p ./mdk"')
    parser.add_argument('--absolute', '-a', required=False, help='format Absolute path example "-a arg"')
