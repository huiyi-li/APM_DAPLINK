...
typedef struct {
    ...
    /* erase sector table (size, base); .size == 0 terminates */
    struct { uint32_t size; uint32_t base; } sectors[APP_ALGO_SECTORS_MAX];
} app_algo_t;

/* Fill `a` with the builtin default algorithm (GD32F470), used when the
