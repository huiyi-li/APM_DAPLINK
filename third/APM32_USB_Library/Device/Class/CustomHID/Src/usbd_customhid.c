...
static USBD_STA_T USBD_CUSTOM_HID_ClassInitHandler(USBD_INFO_T* usbInfo, uint8_t cfgIndex)
{
    ...
    if (usbInfo->devSpeed == USBD_SPEED_FS)
    {
        ...
        usbInfo->devEpIn[usbDevHID->epOutAddr & 0x0F].interval = USBD_CUSTOM_HID_FS_INTERVAL;
    }
    else
    {
        usbInfo->devEpIn[usbDevHID->epInAddr & 0x0F].interval = USBD_CUSTOM_HID_HS_INTERVAL;
        usbInfo->devEpIn[usbDevHID->epOutAddr & 0x0F].interval = USBD_CUSTOM_HID_HS_INTERVAL;
    }
    ...
    usbInfo->devEpIn[usbDevHID->epOutAddr & 0x0F].useStatus = ENABLE;
