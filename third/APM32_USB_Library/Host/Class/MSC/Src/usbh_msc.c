    ...
    switch (reqStatus)
    {
        case USBH_OK:
            if (usbHostMSC->storage[lun].changeState == ENABLE)
            {
                ...
                USBH_USR_LOG("MSC device capacity     : %u bytes", (uint32_t)(blockNum * blockSize));

                USBH_USR_LOG("MSC device block number : %u", blockNum);

                USBH_USR_LOG("MSC device block size   : %u", blockSize);
            }
...
static USBH_STA_T USBH_MSC_CoreHandler(USBH_INFO_T* usbInfo)
{
    ...
    if (usbHostMSC->state == USBH_MSC_INIT)
    {
        if (usbHostMSC->curLun >= usbHostMSC->maxLun)
        {
            ...
            USBH_USR_Debug("%s is launched", usbInfo->hostClass->className);
